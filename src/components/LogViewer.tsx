...
const LogViewer = () => {
  ...
  useEffect(() => {
    localStorage.setItem('favoriteLogGroups', JSON.stringify(favorites));
  }, [favorites]);
  ...
  const toggleFavorite = (logGroupName: string) => {
    setFavorites((prev) =>
      prev.includes(logGroupName)
        ? prev.filter((g) => g !== logGroupName)
        : [...prev, logGroupName]
    );
  };
            ...
            <List dense>
              {favorites.map((fav) => (
                <ListItem
                  key={fav}
                  component="button"
                  onClick={() => setSelectedGroup(fav)}
                  secondaryAction={
                    <IconButton edge="end" onClick={() => toggleFavorite(fav)} size="small">
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, width: '100%' }}>
                      ...
                      <IconButton
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleFavorite(group.logGroupName);
                        }}
                        sx={{ ml: 1 }}
                      >
                        {favorites.includes(group.logGroupName) ? (
                          <StarIcon color="warning" fontSize="small" />
                        ) : (
